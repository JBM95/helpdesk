...
- **Should `role` be writable on a principal that cannot sign in?** Open. `PUT /api/users/:id` does not apply the `deletedAt: null` and `id != AI_AGENT_ID` filters that `GET /api/users` does, so an admin can promote the AI pseudo-user or a soft-deleted user. Neither can authenticate, so nothing is escalated — but the row then holds an unintended role and becomes undeletable, since the delete guard reads the stored role. A guard was written and reverted during GH-8 as out-of-scope; the choice between refusing just the role change and treating those rows as 404 for the whole `PUT` is recorded in [[tech-debt|TD-22]].
