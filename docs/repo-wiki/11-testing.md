...
### `users.spec.ts` — 30 tests across 14 describes

> 7 of these predate GH-8; the 23 in the `Role management` describe arrived with it and are
...
**What exists:**
- `UsersPage.test.tsx:190` — the delete button is absent on admin rows. The only *static* test of role-conditional UI; since GH-8 the E2E demotion scenario also asserts the same button vanishing on promotion and returning on demotion.
